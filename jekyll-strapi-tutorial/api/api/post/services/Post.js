...
module.exports = {
  ...
  add: async (values) => {
    const data = await Post.create(_.omit(values, _.keys(_.groupBy(strapi.models.post.associations, 'alias'))));
    await strapi.hook.mongoose.manageRelations('post', _.merge(_.clone(data), { values }));
    return data;
  },
  ...
  edit: async (params, values) => {
    ...
    await strapi.hook.mongoose.manageRelations('post', _.merge(_.clone(params), { values }));
    return Post.update(params, values, { multi: true });
  },
  ...
  remove: async params => {
    ...
    const data = await Post.findOneAndRemove(params, {})
      .populate(_.keys(_.groupBy(_.reject(strapi.models.post.associations, {autoPopulate: false}), 'alias')).join(' '));
